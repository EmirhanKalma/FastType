const dictionary = "light ocean voice table water heart earth apple field dance music spoon front owner uncle stone paint daily young juice visit space paper clear image after frame".split(" ");
...
// Генерация клавиатуры (привязка к темам)
const initKeyboard = () => {
  rows.forEach((row, i) => {
    ...
    [...row].forEach(letter => {
      ...
      btn.onclick = () => handleInput(letter.toLowerCase());
      rowDiv.appendChild(btn);
    });
  });
  document.getElementById("key-space").onclick = () => handleInput(" ");
};

// Обновление курсора (твой любимый метод)
const updateCursor = () => {
  let spans = wordsWrapper.getElementsByTagName("span");
  let current = spans[charIndex];
  if (current && cursor) {
    let spanRect = current.getBoundingClientRect();
    let wrapperRect = wordsWrapper.getBoundingClientRect();
...
const generateWords = () => {
  ...
  for (let i = 0; i < 60; i++) {
    let txt = dictionary[Math.floor(Math.random() * dictionary.length)];
...
const handleInput = (char) => {
  ...
  const spans = wordsWrapper.querySelectorAll("span");
  if (charIndex >= spans.length) return;

  if (char === spans[charIndex].textContent) {
...
const finishRound = () => {
  ...
  let wpm = Math.round((charIndex / 5) / (15 / 60));
